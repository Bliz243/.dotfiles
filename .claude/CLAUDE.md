...
- Global Claude configuration is managed from `~/.dotfiles/.claude/`.
- `~/.claude/CLAUDE.md`, `settings.json`, hooks, statusline, shared config, global skills, and agents should be symlinked from dotfiles.
- Runtime data, auth, history, sessions, caches, local settings, and plugin-managed state stay in `~/.claude/`.
- Keep this file aligned with `~/.dotfiles/.codex/AGENTS.md` for shared behavior. Tool-specific details belong only in the relevant file.
