...
for skill in ~/.dotfiles/agents/skills/*/; do
    for dir in ~/.claude/skills ~/.agents/skills; do
        [[ -e "$dir/$(basename "$skill")/SKILL.md" ]] || { echo "  Skill not linked: $dir/$(basename "$skill")"; AGENT_OK=false; }
    done
done
[[ ! -e ~/AGENTS.md ]] || { echo "  ~/AGENTS.md exists (stow leak)"; AGENT_OK=false; }
if $AGENT_OK; then
    test_result "Agent config linked" "pass"
else
...
# interactive shells still get the eza/zoxide replacements.
echo "Testing: Agent shell defaults..."
AGENT_EXPECTED="ls: command cat: command cd: builtin mkdir: command"
AGENT_OK=true
for marker in CLAUDECODE CODEX_THREAD_ID; do
    got=$(timeout 20 env -u CLAUDECODE -u CODEX_THREAD_ID -u AI_AGENT -u TMUX "$marker=1" \
          zsh -i -c 'whence -w ls cat cd mkdir' 2>/dev/null </dev/null | tr '\n' ' ')
    [[ "${got% }" == "$AGENT_EXPECTED" ]] || { echo "  $marker shell: ${got:-no output (tmux attach?)}"; AGENT_OK=false; }
done
human=$(env -u CLAUDECODE -u CODEX_THREAD_ID -u AI_AGENT TMUX=1 zsh -i -c 'whence -w ls cd' 2>/dev/null </dev/null | tr '\n' ' ')
[[ "${human% }" == "ls: alias cd: function" ]] || { echo "  interactive shell: $human"; AGENT_OK=false; }
if $AGENT_OK; then
    test_result "Agent shells use standard commands" "pass"
else
    test_result "Agent shells use standard commands" "fail"
fi
